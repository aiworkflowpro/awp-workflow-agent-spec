...
def check_json(root: Path, findings: list[Finding]) -> None:
    for path in json_files(root):
        try:
            ...
            add(findings, root, path, f"JSON cannot be parsed: {exc}")
...
def check_markdown_fences(root: Path, findings: list[Finding]) -> None:
    for path in markdown_files(root):
        ...
        if count % 2:
            add(findings, root, path, f"Odd number of code fences: {count}")
...
def check_skill_files(root: Path, findings: list[Finding]) -> None:
    for path in sorted(root.rglob("SKILL.md")):
        ...
        if not meta:
            add(findings, root, path, "Missing YAML frontmatter.")
            continue
        name = meta.get("name", "")
        description = meta.get("description", "")
        if not NAME_RE.match(name):
            add(findings, root, path, "name must use lowercase letters, digits, and hyphens.")
        if len(description) < 30:
            add(findings, root, path, "description is too short to trigger reliably.")
        if "not for" not in description.lower() and "do not" not in text.lower():
            add(findings, root, path, "description or body should state the exclusion scenario.")
        for heading in ("## Scope", "## Input", "## Workflow", "## Output"):
            if heading not in text:
                add(findings, root, path, f"Missing section: {heading}")
...
def check_progress(root: Path, findings: list[Finding]) -> None:
    ...
    for path in candidates:
        ...
        for key in required:
            if key not in data:
                add(findings, root, path, f"progress.json is missing field: {key}")
        if data.get("status") not in {"pending", "processing", "completed", "failed"}:
            add(findings, root, path, "status is not in the allowed set.")
        current_step = data.get("current_step")
        if isinstance(current_step, str) and not STEP_RE.match(current_step):
            add(findings, root, path, "current_step does not match the stepNN-action format.")


def check_trigger_eval(root: Path, findings: list[Finding]) -> None:
    path = root / "evals/trigger-queries.json"
    if not path.exists():
        add(findings, root, path, "Missing trigger eval.")
        return
    data = load_json(path)
    queries = data.get("queries", [])
    if not isinstance(queries, list) or len(queries) < 10:
        add(findings, root, path, "queries needs at least 10 entries.")
        return
    ...
    if len(should) < 5 or len(should_not) < 5:
        add(findings, root, path, "Both positive and negative trigger samples need at least 5 entries.")
    ids = [item.get("id") for item in queries]
    if len(ids) != len(set(ids)):
        add(findings, root, path, "query id has duplicates.")
...
def check_runtime(root: Path, findings: list[Finding]) -> None:
    for path in sorted(root.glob("**/config/runtime.json")):
        ...
        if data.get("schema_version") != "1.0":
            add(findings, root, path, "schema_version must be 1.0.")
        skill = data.get("skill", "")
        if not isinstance(skill, str) or not NAME_RE.match(skill):
            add(findings, root, path, "skill field must be a stable hyphen-case name.")
        for model in data.get("models", []):
            if model.get("required") and not model.get("sha256"):
                add(findings, root, path, f"Required model is missing sha256: {model.get('id')}")
        cache_policy = data.get("cache_policy", {})
        if cache_policy.get("sync") is True:
            add(findings, root, path, "A public Skill's cache_policy.sync should not default to true.")
    ...
    for path in sorted(root.rglob("*")):
        ...
        for pattern in PRIVATE_PATTERNS:
            if pattern.search(text):
                add(findings, root, path, f"Suspected private path or credential pattern: {pattern.pattern}")
...
def check_required_files(root: Path, findings: list[Finding]) -> None:
    ...
    for item in required:
        path = root / item
        if not path.exists():
            add(findings, root, path, "Missing required file.")
